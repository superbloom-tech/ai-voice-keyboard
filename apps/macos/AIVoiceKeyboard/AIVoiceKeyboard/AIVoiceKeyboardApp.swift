...
private struct MenuBarMenuView14: View {
  ...
  var body: some View {
    Group {
      ...
      Button("Toggle Insert Recording") {
        NotificationCenter.default.post(name: .avkToggleInsertRecording, object: nil)
      }
      .keyboardShortcut(.space, modifiers: [.option])

      Button("Toggle Edit Recording") {
        NotificationCenter.default.post(name: .avkToggleEditRecording, object: nil)
      }
      ...
      if let message = model.hotKeyErrorMessage {
        Text("Hotkeys error: \(message)")
      } else {
        Text("Hotkeys: ⌥Space (Insert), ⌥⇧Space (Edit)")
      }

      if let message = model.permissionWarningMessage {
      ...
      Button("Quit") {
        NotificationCenter.default.post(name: .avkQuitRequest, object: nil)
      }
...
private struct MenuBarMenuViewLegacy: View {
  @ObservedObject private var model = AppModel.shared
...
private struct MenuBarLabelView: View {
  ...
  var body: some View {
    Image(systemName: model.status.systemSymbolName)
